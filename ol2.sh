...
setup_config() {
  ...
  if [ ! -f "$ENV_FILE" ]; then
    ...
    echo "OLLAMA_BASE_URL=" >> "$ENV_FILE" # e.g., http://localhost:11434
    # OLLAMA_MODEL is not stored in .env, it's part of the main config file
  ...
  if [ -z "$GEMINI_API_KEY" ] && [ -z "$OPENROUTER_API_KEY" ] && [ -z "$OLLAMA_BASE_URL" ] && [ -z "${OLLAMA_BASE_URL:-}" ]; then # Check if OLLAMA_BASE_URL is truly unset or empty
    echo "Warning: No API keys (Gemini/OpenRouter) or OLLAMA_BASE_URL found in $ENV_FILE. Please add at least one configuration method." >&2
    # Decide if we should exit or continue (maybe allow local-only mode later?)
  fi

  # Set effective Ollama base URL (use default if not set in .env)
  OLLAMA_BASE_URL_EFFECTIVE="${OLLAMA_BASE_URL:-http://localhost:11434}"


  # Load or create config file
  if [ ! -f "$CONFIG_FILE" ]; then
    echo "Creating default settings file: $CONFIG_FILE"
    echo "API_PROVIDER=" >"$CONFIG_FILE" # Will be 'gemini', 'openrouter', or 'ollama'
...
configure_settings() {
  ...
  API_PROVIDER=$(echo -e "$providers" | fzf --prompt="Select API Provider: " --height=5 --layout=reverse) # Use fixed height, increased for Ollama
  ...
  "OpenRouter")
    ...
    ;;
  "Ollama")
    # OLLAMA_BASE_URL_EFFECTIVE is set in setup_config
    echo "Fetching Ollama models from $OLLAMA_BASE_URL_EFFECTIVE..."
    if [ -z "$OLLAMA_BASE_URL" ] && [ "$OLLAMA_BASE_URL_EFFECTIVE" == "http://localhost:11434" ]; then
        echo "Info: OLLAMA_BASE_URL not set in $ENV_FILE, using default: $OLLAMA_BASE_URL_EFFECTIVE" >&2
    fi

    local ollama_response
    ollama_response=$(curl -s "$OLLAMA_BASE_URL_EFFECTIVE/api/tags")
    if [ $? -ne 0 ]; then
      echo "Warning: curl command failed for Ollama ($OLLAMA_BASE_URL_EFFECTIVE). Is Ollama running? Using manual entry only." >&2
    elif echo "$ollama_response" | jq -e '.models' >/dev/null 2>&1; then
      fetched_models=$(echo "$ollama_response" | jq -r '.models[].name')
      if [ -z "$fetched_models" ]; then
        echo "Warning: No models found from Ollama at $OLLAMA_BASE_URL_EFFECTIVE. Ensure models are pulled (e.g., 'ollama pull llama2'). Using manual entry only." >&2
      fi
    else
      echo "Warning: Invalid JSON or missing 'models' in Ollama response. Is Ollama running and reachable at $OLLAMA_BASE_URL_EFFECTIVE? Response: $ollama_response. Using manual entry only." >&2
    fi
    ;;
  *)
...
create_api_message_json() {
  ...
  if [[ "$API_PROVIDER" == "Gemini" ]]; then
    ...
    # Role mapping (e.g. "model" to "assistant") will be handled by the OpenRouter section if a Gemini history item is loaded.
    # System prompt role is handled directly as "system" when constructing the API call.
...
call_api() {
  ...
  for history_item_json in "${CHAT_HISTORY[@]}"; do
    ...
    # Try extracting from Gemini format first, then OpenRouter/Ollama format
  ...
  if [[ "$API_PROVIDER" == "Gemini" ]]; then
    ...
    # OLLAMA_BASE_URL_EFFECTIVE is set in setup_config
    api_url="${OLLAMA_BASE_URL_EFFECTIVE}/api/chat"

    if [ -z "$MODEL" ]; then
        echo "Error: Ollama model not set in $CONFIG_FILE. Please configure using '$0 --config'." >&2
        return 1
    fi

    # Ollama payload structure: { "model": "...", "messages": [...], "stream": false }
    # Create the system message JSON for Ollama
    # The role here must be "system" for Ollama's system prompt handling.
    local ollama_system_message_json=$(create_api_message_json "system" "$SYSTEM_PROMPT")

    # Prepend the system message JSON string to the history_json_array
    # history_json_array already contains user/assistant messages converted via create_api_message_json
    local messages_with_system_ollama=$(echo "$history_json_array" | jq --argjson sys_msg "$ollama_system_message_json" '[$sys_msg] + .')

    payload=$(jq -n --arg model "$MODEL" --argjson messages "$messages_with_system_ollama" \
      '{model: $model, messages: $messages, stream: false}')

    # 3. Make the curl request for Ollama
    response=$(curl -s -X POST "$api_url" \
      -H "Content-Type: application/json" \
      -d "$payload")

    # 4. Parse Ollama response and handle errors
    if [ $? -ne 0 ]; then
      echo "Error: curl command failed for Ollama at $api_url. Is Ollama running?" >&2
      return 1
    fi

    # Check for API errors in the response JSON (Ollama typically puts error in a top-level 'error' field)
    ...
    if [ -z "$response_text" ] && [ -z "$error_message" ]; then # If no error but also no text
      echo "Error: Could not extract text from Ollama response, and no error reported. Check Ollama logs." >&2
      echo "Full Ollama Response: $response" >&2 # for debugging
      return 1
    elif [ -z "$response_text" ] && [ -n "$error_message" ]; then # Error already handled, just ensure response_text isn't used
        return 1
    fi
...
main() {
  ...
  while true; do
    ...
    local response_role="assistant" # Default for OpenRouter & Ollama
    if [[ "$API_PROVIDER" == "Gemini" ]]; then
      response_role="model" # Gemini uses "model" for AI responses
    fi
